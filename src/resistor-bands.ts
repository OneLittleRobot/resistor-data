...
import type { ResistorColor, BandCount, ResistorTolerance, ResistorData } from './types';

export const bandsToNotation = (bands: string[] = []): ResistorData => {
...
export const notationToBands = (data: [string, ResistorTolerance?], bands: BandCount = 5): ResistorColor[] => {
    if (!Array.isArray(data)) {
        throw new TypeError('Expected an array');
    }

    const value = notationToValue(data[0]);
    const tolerance: number = data[1] ?? 1;
    const digits = (value * 1000)
        .toString()
        .split('')
        .slice(0, bands - 2);

    const exp = getExponentialFor10(value / Number(digits.join('')));

    return [
        ...digits.map(d => getColorFromValue(Number(d))),
